<template>
  <div class="select-none">
    ...
    <div class="flex items-center justify-between mb-3.5">
      <span class="text-sm font-bold text-ink">{{ monthLabel }}</span>
    ...
    <div class="grid grid-cols-7 gap-1">
      <div
        ...
        :class="[
          'text-center text-[13px] py-2 rounded-lg font-medium transition-all duration-100 cursor-pointer',
          cell.dim        ? 'text-slate-300 cursor-default'            : '',
          cell.selectable && cell.hasSlots ? 'text-emerald bg-emerald-4 font-bold' : '',
          cell.selectable && !cell.hasSlots && !cell.dim ? 'text-slate-600 hover:bg-slate-100' : '',
          cell.selected   ? '!bg-emerald !text-white shadow-sm'        : '',
          !cell.selectable && !cell.dim ? 'text-slate-300 cursor-not-allowed' : '',
        ]"
      >
        {{ cell.day }}
      </div>
...
const calendarCells = computed((): CalendarCell[] => {
  ...
  for (let d = 1; d <= lastDate; d++) {
    ...
    cells.push({
      ...
      selectable: !isPast && props.datesWithSlots.includes(dateStr),
    })
  }
...
<style scoped>
.cal-nav-btn {
  @apply w-7 h-7 flex items-center justify-center rounded-md border border-slate-200 text-slate-400 hover:border-emerald hover:text-emerald transition-colors text-sm;
}
</style>
