      ...
      <div class="grid grid-cols-[1fr_280px] gap-6 items-start">
        <div>
          ...
          <div class="flex items-center justify-between mb-3">
            <h2 class="text-[13px] font-bold text-ink">Weekly schedule</h2>
            <span class="text-xs text-slate-400">{{ weekLabel }}</span>
          </div>
          <div class="bg-white border border-slate-200 rounded-[14px] shadow-sm overflow-hidden mb-6">
            <!-- Calendar header -->
            <div class="grid bg-slate-50 border-b border-slate-200" :style="gridStyle">
              <div class="px-3 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-r border-slate-200"></div>
              <div
                v-for="day in weekDays"
                :key="day.label"
                class="px-2 py-2.5 text-center text-[11px] font-bold text-slate-400 uppercase tracking-wide border-r border-slate-200 last:border-0"
              >
                {{ day.label }}
              </div>
            </div>
            ...
            <div
              ...
              :key="hour"
              class="grid border-b border-slate-100 last:border-0"
              :style="gridStyle"
            >
              <div class="px-3 py-2 text-[10px] text-slate-400 font-semibold text-right border-r border-slate-100">{{ hour }}</div>
              <div
                v-for="day in weekDays"
                :key="day.date"
                class="p-1.5 border-r border-slate-100 last:border-0 min-h-[48px]"
              >
                <div
                  ...
                  :class="['text-[11px] font-bold rounded-[6px] px-2 py-1.5 cursor-pointer transition-colors',
                    appt.status === 'confirmed'
                      ? 'bg-emerald-4 border border-emerald-3 text-[#065F46] hover:bg-emerald-3'
          ...
          <!-- Pending requests -->
          <div class="flex items-center justify-between mb-3">
            <h2 class="text-[13px] font-bold text-ink">Pending requests</h2>
            <span v-if="pending.length" class="text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200 px-2.5 py-1 rounded-full">
              {{ pending.length }} waiting
            </span>
          </div>
          <div class="bg-white border border-slate-200 rounded-[14px] shadow-sm overflow-hidden">
            <div v-if="pending.length === 0" class="py-10 text-center text-sm text-slate-400">
              No pending requests — you're all caught up ✓
            </div>
            <div
              v-for="appt in pending"
              :key="appt.id"
              class="flex items-center gap-4 px-5 py-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors"
            >
              <div class="text-right min-w-[52px]">
                <div class="text-sm font-bold text-ink">{{ formatTime(appt.scheduled_at) }}</div>
                <div class="text-[10px] text-slate-400">{{ formatShortDate(appt.scheduled_at) }}</div>
              </div>
              <div class="flex-1">
                <div class="text-sm font-semibold text-ink">{{ appt.patient.name }}</div>
                <div class="text-xs text-slate-400 mt-0.5">{{ appt.reason ?? 'No reason specified' }}</div>
              </div>
        ...
        <div class="space-y-5">
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              <h2 class="text-[13px] font-bold text-ink">Weekly activity</h2>
            ...
            <div class="bg-white border border-slate-200 rounded-[14px] shadow-sm p-5">
              <div class="text-xs text-slate-400 mb-3.5">Appointments / day</div>
          ...
          <div>
            <h2 class="text-[13px] font-bold text-ink mb-3">Availability</h2>
            <div class="bg-white border border-slate-200 rounded-[14px] shadow-sm p-5">
              <div class="form-group">
                <label class="form-label">Consultation fee</label>
                <div class="relative">
                  <input v-model="settingsForm.fee" class="form-input pr-14" type="number">
                  <span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">MAD</span>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="flex flex-wrap gap-1.5 mt-1">
                  <button
                    ...
                    :class="['text-xs font-bold px-3 py-1.5 rounded-md border transition-all',
              ...
              <button
                @click="saveSettings"
                class="w-full py-2.5 bg-emerald text-white text-sm font-bold rounded-[10px] hover:bg-emerald-2 transition-colors shadow-sm"
              >
                Save schedule
              </button>
            </div>
